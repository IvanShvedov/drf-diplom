    ...
    def __init__(self, serializer, response_to: str):
        try:
            if 'vacancy' in response_to:
                response = VacancyResponse.objects.filter(vacancy=serializer['vacancy'], employer=serializer['employer']).first()
                ...
                response = CvResponse.objects.filter(cv=serializer['cv'], worker=serializer['worker']).first()
